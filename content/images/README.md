...
글자가 들어가는 카드는 **AI 이미지 생성이 한글을 제대로 못 쓴다.**
글자를 넣으라고 하면 깨진 문자가 나오고, 넣지 말라고 하면 빈 카드가 나온다.

그래서 `posts-data.json` 의 8·9번 이미지에 `"card": true` 를 달아
`gen-images.mjs` 가 건너뛰게 했다. 이 둘은 `cards.html` 로 뽑아
`content/images/<슬러그>/08.png`, `09.png` 로 넣는다.

`--include-cards` 로 강제할 수는 있지만 권하지 않는다.
