...
멈춘 이유: 키를 넣기 전에 예약부터 걸어두는 바람에, 매시간 실패하면서 GitHub 실패 알림
메일이 계속 나갔다. **다시 켤 때는 순서를 지킨다 — 키를 먼저 넣고, 수동으로 돌려보고,
결과를 확인한 다음에 예약을 건다.** 반대로 하지 않는다.
...
`content/threads/voice.md` 가 정본이다. 말투를 바꿀 때 코드를 고치지 말고 이 파일을 고친다.

사람이 보지 않는 상태로 글이 나가므로 안전장치가 마지막 문이다.

- `scripts/threads/guard.mjs` 를 통과하지 못하면 **올리지 않는다.** 경고로 낮추지 않는다.
- 상태 파일을 두지 않는다. "이미 답했는가", "오늘 몇 개 올렸는가" 는 스레드 API 를
  되읽어 판단한다. 러너가 매번 새로 떠도 두 번 답하지 않는다. 이 구조를 파일 기반으로
  바꾸지 않는다 — 러너에 파일이 남지 않는다.
- 애매한 댓글은 답하지 않고 `threads-hold` 이슈로 사람에게 넘긴다. 자동 답변 범위를
  넓히는 것은 사용자가 정할 일이다.
...
돌려보지 않고 "발행했습니다"라고 보고하지 말 것.

## 검수 도구

원고 검수기는 **`scripts/lint-post.mjs` 하나만 게이트다.**
...
2,310자 / 2,854자로 다르게 센다. 다만 lint-post.mjs 에 없는 검사 — 광고 심의 금지어,
출처 URL, 이미지 장수, Q&A 개수 — 를 갖고 있어 남겨둔다. 쓰려면 앞머리 파서와
분량 계산을 먼저 맞춰야 한다.

**두 검수기를 동시에 신뢰하지 않는다.** 발행 여부는 `lint-post.mjs` 로만 판단한다.
